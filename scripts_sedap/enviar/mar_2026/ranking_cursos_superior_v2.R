...
for (ano in 2009:2022) {
  ...
  COPY (
    SELECT
      ...
      %d AS ano_enem,
      (
        COALESCE(NU_NOTA_CN, NOTA_CN, NU_NT_CN) +
        COALESCE(NU_NOTA_CH, NOTA_CH, NU_NT_CH) +
        COALESCE(NU_NOTA_LC, NOTA_LC, NU_NT_LC) +
        COALESCE(NU_NOTA_MT, NOTA_MT, NU_NT_MT) +
        COALESCE(NU_NOTA_REDACAO, NOTA_REDACAO)
      ) / 5.0 AS nota_enem,
      (hash(CPF_MASC) %% 128 + 1) AS bucket
    FROM read_csv('%s', encoding = 'latin-1', union_by_name = TRUE)
    WHERE
      CPF_MASC IS NOT NULL 
      AND COALESCE(NU_NOTA_CN, NOTA_CN, NU_NT_CN) IS NOT NULL
      AND COALESCE(NU_NOTA_CH, NOTA_CH, NU_NT_CH) IS NOT NULL
      AND COALESCE(NU_NOTA_LC, NOTA_LC, NU_NT_LC) IS NOT NULL
      AND COALESCE(NU_NOTA_MT, NOTA_MT, NU_NT_MT) IS NOT NULL
      AND COALESCE(NU_NOTA_REDACAO, NOTA_REDACAO) IS NOT NULL
  )
  TO '%s'
