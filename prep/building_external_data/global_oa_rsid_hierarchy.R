# Offline LOCAL prep, using the local global OA snapshot and local education only.
# Separate comparison products; never publishes over production or audit files.
# Run from repository root: Rscript prep/building_external_data/global_oa_rsid_hierarchy.R
# Re-running resumes completed, checksummed decision partitions and pipeline phases.
library(DBI)
library(duckdb)
library(jsonlite)
library(countrycode)
...
out <- Sys.getenv('OBMEP_GLOBAL_OA_OUT',file.path(coh,'global_oa_hierarchy'))
dir.create(out,recursive=TRUE,showWarnings=FALSE)
dir.create(file.path(out,'checkpoints'),showWarnings=FALSE)
dir.create(file.path(out,'decisions'),showWarnings=FALSE)
dir.create(file.path(out,'candidate_evaluations'),showWarnings=FALSE)
dir.create(file.path(out,'education_parts'),showWarnings=FALSE)
snapshot <- 'C:/Users/megaj/Globtalent Dropbox/GTAllocation/Data/external/oa_snapshot/data/institutions'
source_path <- file.path(coh,'obmep_candidates_step_1_br_openalex_rsid_nofloor_matches.parquet')
br_path <- file.path(root,'Data/intermediate/openalex_institutions/openalex_institutions_br.parquet')
...
sh_path <- file.path(root,'Data/intermediate/shanghai_ranking/shanghai_ranking_oa_parents.parquet')
snapshot_paths <- list.files(snapshot,pattern='[.]gz$',recursive=TRUE,full.names=TRUE)
input_paths <- unique(c(source_path,br_path,ruf_paths,sh_path,snapshot_paths,
 list.files(file.path(coh,'obmep_candidates_step_1_education'),full.names=TRUE)))
stopifnot(length(snapshot_paths)>0,all(file.exists(input_paths)))
code_manifest <- file.path(out,'implementation_manifest.rds')
if(file.exists(code_manifest)) {
 previous_code <- readRDS(code_manifest)
 stopifnot(identical(previous_code$md5,unname(tools::md5sum(previous_code$path))))
}
cat('Checking input and preservation checksums.\n')
inputs <- data.frame(path=input_paths,md5=unname(tools::md5sum(input_paths)))
manifest <- file.path(out,'input_manifest.rds')
if(file.exists(manifest)) stopifnot(identical(inputs,readRDS(manifest))) else saveRDS(inputs,manifest)
protected_path <- file.path(out,'preservation_manifest.rds')
if(!file.exists(protected_path)) {
 protected <- c(list.files(coh,full.names=TRUE),
  list.files(file.path(root,'Data/intermediate/openalex_institutions'),recursive=TRUE,full.names=TRUE),
  list.files(file.path(root,'Data/intermediate/shanghai_ranking'),recursive=TRUE,full.names=TRUE),
  list.files('outputs',recursive=TRUE,full.names=TRUE))
 protected <- unique(protected[!dir.exists(protected)])
 protected <- protected[!grepl('global_oa_hierarchy',protected,fixed=TRUE)]
 saveRDS(data.frame(path=protected,md5=unname(tools::md5sum(protected))),protected_path)
}
protected <- readRDS(protected_path)
protected <- protected[!grepl('global_oa_hierarchy',protected$path,fixed=TRUE),]
# The prior rebuild froze checksums for partitioned position and selection files.
prior_report <- read_json(file.path(coh,'education_flags_rebuild_report.json'),simplifyVector=TRUE)
extra_protected <- prior_report$protected_products
extra_protected <- extra_protected[!extra_protected$path %in% protected$path,]
if(nrow(extra_protected)>0) protected <- rbind(protected,extra_protected[,c('path','md5')])
saveRDS(protected,protected_path)
...
dir.create(file.path(out,'spill'),showWarnings=FALSE)
dbExecute(con,sprintf("SET temp_directory='%s'",file.path(out,'spill')))
dbExecute(con,"CREATE OR REPLACE MACRO regexp_like(s,p) AS regexp_matches(s,p)")
dbExecute(con,sprintf("CREATE OR REPLACE VIEW education AS SELECT * FROM read_parquet('%s')",source_path))
...
if(!file.exists(phase)) {
 ...
 # Kosovo is supplied by OA using its conventional user-assigned ISO code.
 ...
 stopifnot(dbGetQuery(con,'SELECT count(*) n,count(DISTINCT oa_id) ids FROM catalog')$n==120658,
 ...
 for(tbl in c('catalog','aliases','country_mapping')) dbExecute(con,sprintf(
  "COPY %s TO '%s/%s.parquet' (FORMAT PARQUET,COMPRESSION ZSTD)",tbl,out,tbl))
 saveRDS(TRUE,phase)
}

phase <- file.path(out,'checkpoints','02_crosswalk.rds')
if(!file.exists(phase)) {
 cat('Building whole-name and segment evidence from local education rows.\n')
 ...
 for(tbl in c('crosswalk','crosswalk_evidence','raw_links','raw_names','rsid_sets','raw_sets'))
  dbExecute(con,sprintf("COPY %s TO '%s/%s.parquet' (FORMAT PARQUET,COMPRESSION ZSTD)",tbl,out,tbl))
...
if(!file.exists(phase)) {
 cat('Collapsing reusable decisions by original raw text, country, and candidate set.\n')
 ...
 dbExecute(con,sprintf("COPY decisions_input TO '%s/decision_inputs.parquet' (FORMAT PARQUET,COMPRESSION ZSTD)",out))
 saveRDS(TRUE,phase)
}
...
if(!file.exists(phase)) {
 cat('Writing original education rows with global candidates and selections, by source file.\n')
 ...
 for(i in seq_along(files)) {
  ...
  # Every pre-existing value, including physical identities and previous arrays.
...
source('prep/building_external_data/global_oa_hierarchy_flags.R')
source('prep/building_external_data/global_oa_hierarchy_verify.R')
dbDisconnect(con,shutdown=TRUE)
cat('Completed separate global hierarchy comparison:',out,'\n')
